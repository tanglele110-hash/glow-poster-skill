...
2026-09-25：按提供者明确要求新增四张已完成的署名海报，位于 `docs/examples/`，用于 README 的两行两列展示。原始文件直接复制，未重新生成或改变图像；示例不作为 Skill 的默认组合或默认姓名。本次展示范围已获得单独授权。
...
2026-09-25：README 组合库新增六色配色图和五种花窗轮廓图；色值、名称与 ID 按 catalog.json 核对，窗形为原创几何示意。
